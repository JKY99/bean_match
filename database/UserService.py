...
class UserService:
    ...
    async def create(cls, user: User) -> User:
        ...
        created_user = await cls.collection.find_one({"user_id": new_user.inserted_id})
    ...
    async def update(cls, user_id: str, user: User) -> User:
        user_dict = jsonable_encoder(user)
        await cls.collection.replace_one({"user_id": user_id}, user_dict)
        updated_user = await cls.collection.find_one({"user_id": user_id})
        if updated_user:
    ...
    async def delete(cls, user_id: str) -> None:
        delete_result = await cls.collection.delete_one({"user_id": user_id})
